<template>
	<div class="diy_details_box diy_div_message_box">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "message_box_id",
					source_id: this.obj["message_box_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/message_box/set?message_box_id=' + _this.obj["message_box_id"], {
								praise_len
							}, (res) => {
								if(res.result){
