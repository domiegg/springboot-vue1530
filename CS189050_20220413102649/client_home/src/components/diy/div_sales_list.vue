<template>
	<div class="diy_details_box diy_div_sales_list">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "sales_list_id",
					source_id: this.obj["sales_list_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/sales_list/set?sales_list_id=' + _this.obj["sales_list_id"], {
								praise_len
							}, (res) => {
								if(res.result){
