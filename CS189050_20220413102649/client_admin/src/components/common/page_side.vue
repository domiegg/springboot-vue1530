<template>
	<el-menu class="side el-menu-vertical-demo" default-active="/" @open="handleOpen" @close="handleClose"
		...
		<el-submenu index="user"
			...
			<el-menu-item index="/registered_user/table" v-show="user_group == '管理员' || $check_action('/registered_user/table')"><span>注册用户</span></el-menu-item>
		...
		<el-submenu index="mall" v-show="user_group == '管理员' || $check_group(['/goods/table','/order/table'])">
			<template slot="title"><i class="el-icon-goods"></i><span>商城管理</span></template>
			<el-menu-item v-show="user_group == '管理员' || $check_action('/book_mall/table')" index="/book_mall/table"
						  class="bg-hover"><span>图书商城列表</span></el-menu-item>
