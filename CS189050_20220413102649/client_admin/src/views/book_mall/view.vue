			...
			<el-col v-if="user_group === '管理员' || $check_field('get','author') || $check_field('add','author') || $check_field('set','author')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="作者" prop="author">
					<el-input id="author" v-model="form['author']" placeholder="请输入作者"
							  v-if="user_group === '管理员' || (form['book_mall_id'] && $check_field('set','author')) || (!form['book_mall_id'] && $check_field('add','author'))" :disabled="disabledObj['author_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','press') || $check_field('add','press') || $check_field('set','press')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="出版社" prop="press">
					<el-input id="press" v-model="form['press']" placeholder="请输入出版社"
							  v-if="user_group === '管理员' || (form['book_mall_id'] && $check_field('set','press')) || (!form['book_mall_id'] && $check_field('add','press'))" :disabled="disabledObj['press_isDisabled']"></el-input>
	...
	export default {
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/book_mall/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/book_mall/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/book_mall/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/book_mall/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/book_mall/view','get');
            ...
            submit_after(){
                ...
                if (this.form[this.field]){
                    ...
                    this.$get("~/api/book_mall/get_obj?",query_form,(res)=>{
            ...
            submit_goods(source_id,add_flag){
				let customize_field = []
				customize_field.push({"field_name":"作者","field_value":this.form.author});
				customize_field.push({"field_name":"出版社","field_value":this.form.press});
                this.goods_form = {
                    goods_id: 0,
                ...
                if (add_flag==='set'){
                    ...
                    // 提交事件
                    this.$post("~/api/goods/set?source_table=book_mall&source_field=" + this.field + "&source_id=" + source_id,this.goods_form,(res)=>{
